...
const UserrefarrelTable = () => {
  const ApiAllData = useSelector((state) => state.doWin.ApiAllData);
  const [searchsponser, setsearchsponser] = useState([]);
  const [searchsponserId, setsearchsponserId] = useState("");
  const referralData = searchsponserId
    ? searchsponser
    : ApiAllData?.data?.reffereral_user || [];
  ...
  const invoices = referralData.map((item) => {
    ...
    const income1 = formatAmount(item?.income, 2);
    return {
      name: item?.user_name,
      userId: item?.userid,
      income: income1,
      ...
      <div className="max-h-screen  py-4 lg:px-10 px-3">
              ...
              <li aria-current="page">
                <div class="flex items-center">
                  ...
                  <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">
                    {searchsponserId ? searchsponserId : "  My Sponsors"}
                  </span>
                </div>
              </li>
            </ol>
        ...
        <div className="overflow-x-auto px-2 py-4 md:mt-5 ">
          ...
          <div className="overflow-auto">
            ...
            {searchsponser.length === 0 && searchsponserId !== "" && (
...
export default UserrefarrelTable;
