...
import { setpool_levelDashboard } from "../../../Redux/DoWinSlice";

const PartnerTeam = () => {
  const ApiAllData = useSelector((state) => state.doWin.ApiAllData);
  const levelsRawData = ApiAllData?.data?.partnerHeightLevels || [];
          ...
          <table className="min-w-full table-auto border-collapse">
            ...
            <tbody>
              {paginatedInvoices?.map((row, rowIndex) => {
                ...
                if (values?.pool_level == 1) {
                  dispatch(setpool_levelDashboard(total));
                } 
                return (
                  <tr
                    key={rowIndex}
