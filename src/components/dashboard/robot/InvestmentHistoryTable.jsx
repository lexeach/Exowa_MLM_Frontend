...
const InvestmentHistoryTable = () => {
  ...
  const referralData = ApiAllData?.data?.investment_history || [];
  const invoices = referralData.map((item, index) => {
    const formattedAmount = incomeToFixedFun(item?.amount);
    const formattedAmountdoller = incomeToFixedFun(item?.amount_dollar);
    const formattedprofitamount = incomeToFixedFun(item?.profit_amount);

    const datetime = formatsecendDate(item?.date_time);
    return {
      id: index + 1,
      date: datetime,
      userid: item?.userid,
      ...
      invest_trx_id: item?.invest_trx_id,
      dolleramount: formattedAmountdoller,
      status: item?.status,
    };
  });
