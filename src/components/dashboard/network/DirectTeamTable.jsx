...
const DirectTeamTable = () => {
  ...
  const referralData = ApiAllData?.data?.referral || [];
  const levelOneReferralData = referralData.filter((item) => item?.level === 1);
  const invoices = levelOneReferralData.map((item, index) => {
    const formattedAmount = incomeToFixedFun(item?.business_amount);
    const datetime = formatDate(item?.registration_date);
    ...
    return {
      ...
      userid: item?.userid,
      directreferral: item?.direct_referral,
      reffereralcode: item?.reffereral_code,
    };
  });
  ...
  const filteredInvoices = invoices
    .filter(
      (invoice) =>
        ...
        invoice.userid.toLowerCase().includes(searchTerm) ||
          ...
          {currentInvoices.map((invoice) => (
            ...
            >
              ...
              <td
                ...
                style={{ color: Theme.textcolor }}
              >
                {invoice.reffereralcode}
              </td>
              <td
                className="px-4 py-3.5 text-sm text-gray-600"
                style={{ color: Theme.textcolor }}
              >
                {invoice.directreferral}
              </td>
              <td
                className="px-4 py-3.5 text-sm text-gray-600"
                style={{ color: Theme.textcolor }}
